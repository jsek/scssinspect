var BaseReporter, chalk, path, pluralize, pluralizeE, util;
...
pluralizeE = function(count, text) {
  if (count !== 1) {
    return text + 'es';
  } else {
    return text;
  ...
  BaseReporter.prototype._registerSummary = function() {
    return this._inspector.on('end', (function(_this) {
      return function() {
        ...
        skipped = _this._skipped ? " (" + _this._skipped + " " + (pluralize(_this._skipped, 'file')) + " skipped)" : '';
        if (!_this._found) {
          found = chalk.black.bgGreen(" No matches found across " + checked + " ");
        } else {
          found = chalk.white.bgRed(" " + _this._found + " " + (pluralizeE(_this._found, 'match')) + " found across " + checked + " ");
        }
