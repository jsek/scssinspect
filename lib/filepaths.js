var checkIgnorePatterns, checkSuffix, file, findLocalIgnorePatterns, fs, getFilePath, mergeIgnorePatterns, path, sep;
...
findLocalIgnorePatterns = function(localIgnorePatterns, dirPath, files, specialFileName) {
  if (files.filter(function(f) {
    ...
    return localIgnorePatterns[dirPath] = fs.readFileSync(path.normalize(dirPath + '/' + specialFileName), {
      encoding: 'utf8'
    }).replace('**', '.*').replace('/*.', '/.*.').split('\n').map(function(i) {
      return path.normalize(i.trim());
    }).filter(function(i) {
      return i.length;
    }).map(function(i) {
      return new RegExp(i);
...
exports.getSync = function(paths, opts) {
  var globalIgnorePatterns, j, len, localIgnorePatterns, ref, results;
  paths = paths || [];
  opts = opts || {};
  results = [];
  localIgnorePatterns = {};
  globalIgnorePatterns = ((ref = opts.ignore) != null ? ref.map(function(i) {
    return new RegExp(i);
  }) : void 0) || [];
